package com.coroutines.common.preferences
 ...
 *
 * @param key A [String] key which will be used as the name of the preference.
 * Must be unique.
...
/**
 * Returns the default entry of an [EnumPreference] or the first if a default
 * has not been set.
...
inline fun <reified T> defaultOf(): T where T : Enum<T>, T : EnumPreference {
    ...
    ?: throw EmptyEnumPreferenceException(T::class.java.simpleName)
}

class EmptyEnumPreferenceException(enumName: String) : Exception(
    "Enum $enumName has no entries and thus cannot be used as a preference."
)