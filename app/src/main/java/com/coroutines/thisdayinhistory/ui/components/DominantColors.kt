...
import com.coroutines.thisdayinhistory.ui.utils.isDark
...
class DominantColorState(
    ...
    private fun color() : Color {
        val colorSpace = color.colorSpace
        val isDark = color.isDark

        val luminance = color.luminance()
        return when {
            luminance < 0.1f -> color.lighter(0.4f)
            luminance < 0.16f -> color.lighter(0.25f)
            luminance < 0.2f -> color.lighter(0.15f)
            luminance < 0.3f -> color.darker(0.08f)
            luminance > 0.3f && luminance < 0.5f -> color.darker(0.2f)
            luminance > 0.5f && luminance < 0.7f -> color.darker(0.25f)
            else -> color.darker(0.4f)
        }
    }
